export const setupFormValid = () => {
  ...
  Array.from(document.getElementsByTagName("form")).forEach(el => {
    ...
    window._FORM_[el.name] = {};
    window._FORMSTATUS_[el.name] = false;
    window._SELECT_[el.name] = {};
    ...
    Array.from([".pretty > input", "select"]).forEach(tag =>
      Array.from(el.querySelectorAll(tag)).forEach((element: HTMLInputElement) => {
        if (element.type.indexOf("select") > -1)
          window._SELECT_[el.name][element.name] = new Choices(element, {
...
const submitForm = (el: HTMLFormElement) => {
  ...
  window._FORMSTATUS_[el.name] = el.querySelectorAll(".group.error").length === 0;
  return true;
};
const showMessage = (
  el: HTMLInputElement,
  ...
  if (parentElement) {
    if (!isSubmit) {
      if (tag === "select")
        window._FORM_[nameForm][el.name] =
          el.type.indexOf("multiple") > -1
            ? window._SELECT_[nameForm][el.name].getValue().map(i => i.value)
            : window._SELECT_[nameForm][el.name].getValue().value;
      else
        window._FORM_[nameForm][el.name.replace("[]", "")] =
          el.name.indexOf("[]") === -1
            ? el.value
            : [].filter
    ...
    if (message) {
      if (error) error.innerHTML = message;
        ...
        if (tag === "select") window._SELECT_[nameForm][el.name].destroy();
        ...
        Array.from(parentElement.querySelectorAll(tag)).forEach(
          (input: HTMLInputElement, index) => {
            if (tag === "select") {
              window._SELECT_[nameForm][input.name] = new Choices(input, {
                removeItemButton: true,
              });
            }
...
const getMessage = (el: HTMLInputElement, nameForm: string, isSubmit: boolean) => {
  ...
  if (isSubmit || (type === "checkbox" && name && name.indexOf("[]") > -1))
    value = window._FORM_[nameForm][name.replace("[]", "")] as string;
  if (!value && required && name.indexOf("[]") === -1) return window._MESSAGE_.required;
  else if (value && type === "email" && !regexEmail.test(value.trim()))
    return window._MESSAGE_.email;
  else if (
    value &&
    dataset.hasOwnProperty("minLength") &&
    value.length < parseInt(dataset["minLength"]!)
  )
    return window._MESSAGE_.minLengthCheckBox + dataset["minLength"] + " ký tự";
  else if (
    value &&
    dataset.hasOwnProperty("maxLength") &&
    value.length > parseInt(dataset["maxLength"]!)
  )
    return window._MESSAGE_.minLengthCheckBox + dataset["maxLength"] + " ký tự";
  else if (
    value &&
    dataset.hasOwnProperty("regex") &&
    !new RegExp(dataset["regex"]!).test(value.trim())
  )
    return dataset.hasOwnProperty("message") ? dataset["message"] : window._MESSAGE_.required;
  ...
  if (
    ...
    return window._MESSAGE_.minLengthCheckBox + dataset["minLength"];
  return "";
};
