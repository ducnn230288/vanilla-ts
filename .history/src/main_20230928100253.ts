...
setupSwiper2('cate', 2, false, false, { slidePer_xl: 10, slidePer_lg: 6, slidePer_md: 4, slidePer_sm: 3 }, true, false);
setupSwiper2('banner', 1, true, 3000, false, true, false);
setupSwiper2('sale', 1, false, 1000, { slidePer_xl: 3, slidePer_lg: 3, slidePer_md: 2, slidePer_sm: 1 }, true, false);
const swiperList: any = new Swiper('.mySwiper-list', {
  spaceBetween: 10,
  // loop: true,
  slidesPerView: 4,
...
const swiperMain = setupSwiper2('zoom', 1, false, false, false, true, swiperList);

swiperMain.on("slideChangeTransitionStart", function () {
  galleryThumbs.slideTo(swiperMain.activeIndex);
});

swiperList.on('transitionStart', function(){
  swiperMain.slideTo(swiperList.activeIndex);
});
